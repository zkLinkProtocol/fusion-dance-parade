...
import { checkOkx } from 'constants/api';
import FromList from 'constants/from-chain-list';
import { IERC20MetadataFactory } from 'constants/typechain';
import { defaultNetwork } from 'features/bridge/hooks/useBridge';

export const L2_BRIDGE_ABI = new utils.Interface((await import('../constants/contracts/abis/IL2Bridge.json')).abi);
...
export async function getERC20BridgeCalldata(
  ...
  return L2_BRIDGE_ABI.encodeFunctionData(isMergeSelected ? 'finalizeDepositToMerge' : 'finalizeDeposit', [
    l1Sender,
    l2Receiver,
    l1TokenAddress,
    amount,
...
export const formatNumber = (number: number, locales?: string, ops?: Intl.NumberFormatOptions) => {
